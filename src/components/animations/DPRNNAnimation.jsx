...
const DPRNNAnimation = () => {
  ...
  // Reset animation state when mounted
  useEffect(() => {
    ...
    // Short delay to ensure clean animation start
    setTimeout(() => {
      ...
      // Animation sequence with cleanup safeguards
    ...
    <div className="w-full h-48 flex flex-col items-center justify-center">
      <div className="relative w-full h-32">
        ...
        <div className="absolute bottom-0 left-0 w-full flex justify-around text-center">
          <motion.div 
            className={`text-[7px] font-bold ${processingStage === 1 || processingStage === 3 ? 'text-[#ff5722]' : 'text-gray-400'}`}
            animate={{ opacity: processingStage >= 1 ? 1 : 0.3 }}
          >
            Intra-RNN
          </motion.div>
          <motion.div 
            className={`text-[7px] font-bold ${processingStage === 2 || processingStage === 3 ? 'text-[#2196f3]' : 'text-gray-400'}`}
            animate={{ opacity: processingStage >= 2 ? 1 : 0.3 }}
          >
            Inter-RNN
          </motion.div>
          <motion.div 
            className={`text-[7px] font-bold ${processingStage === 3 ? 'text-[#9c27b0]' : 'text-gray-400'}`}
      ...
      {processingStage === 3 && (
        <motion.div 
          className="mt-1 px-2 py-0.5 bg-green-900 rounded-sm text-[7px] text-green-300 text-center"
